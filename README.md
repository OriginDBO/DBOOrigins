<div align="center">

# 🐉 DBO Unbound  

**A Dragon Ball Online Private Server Project**

**Based Off [DBOGLOBAL](https://github.com/dboglobal/DBOGLOBAL).**

*Built from the ground up. Driven by passion. Powered by community.*

[![Discord](https://img.shields.io/badge/Discord-Join%20Us-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/fnRr3pBWg6)
[![Status](https://img.shields.io/badge/Status-In%20Development-yellow?style=for-the-badge)]()

</div>

---

## 🌐 About DBO Unbound

DBO Unbound is a private Dragon Ball Online server project — a labor of love from a small, tight-knit team that got tired of waiting and decided to build something themselves.

We're not a massive organization. We're a small group of friends who care deeply about this game and the community around it. That's exactly what makes us different. Every decision, every line of code, every piece of content — it's made with intention, with care, and with our players in mind.

We started with nothing but publicly available resources and a shared drive to make progress where others hadn't. Over time, that turned into real achievements: custom skills with new animations and effects, new bosses, new systems, and content that hadn't been seen publicly since the original game went offline.

We've been at this since **April 2021** — and we're not going anywhere.

---

## 🎯 Our Purpose

The goal was simple from the start: **do what everyone's afraid of doing**

The DBO private server scene had no shortage of promises, but progress was slow, public, and rarely delivered. We chose a different path — heads down, focused, building in the background, and letting the work speak for itself.

We launched **DBOGenerations** in 2021 using the DBO TW 1.69 client as a base. Months later we shifted focus to the **DBO KR 2.0 client** — a significantly more complex undertaking — and began delivering things that hadn't been done publicly in the decade since the original servers shut down.

Some of what we're proud of:

- 🥋 **Brand new skills** for all classes — custom animations, effects, and sounds
- 🏃 **New character animations** — fly idle, fast fly, dash, dances, and more
- 👹 **New bosses** — completely original animations and skill sets
- 📦 **Custom content pipeline** — built and refined entirely in-house

This project — **DBO Unbound** — is the next chapter. A fresh start with everything we've learned, built the right way, with full control over our own codebase.

---

## 💬 Community

The community is at the heart of everything we do. Our players stuck with us through every challenge, and that support means more to us than anything.

We made a promise to ourselves early on: **If We're Gonna Do It, Do It Right** If the day ever comes that we have to stop, it won't be without warning, and it won't be without good reason. Until then — we're here, we're building, and we're grateful for every person who believes in what we're doing.

**Join us on Discord:**

[![Discord Banner](https://img.shields.io/badge/Discord-DBO%20Unbound-5865F2?style=flat-square&logo=discord&logoColor=white)](https://discord.gg/fnRr3pBWg6)

---

## 📚 Documentation

Project documentation is maintained on the **GitHub Wiki** and will grow as development progresses.

> 📖 **[View the Wiki →](../../wiki)**

Topics will include server setup guides, content documentation, development notes, and more as they become available.

---

## 🗺️ Project Timeline

| Period | Milestone |
|---|---|
| April 2021 | Team officially formed |
| December 2021 | Development gets serious — real work, real player base |
| Late 2021 | Major DBO content breakthroughs — first publicly done in over a decade |
| February 2022 | DBOGalaxy Alpha launched (TW 1.69 client) |
| Mid 2022 | Shifted to DBO KR 2.0 client development |
| Late 2022 | Team and experience grows — preparation for 2.0 public release begins |
| February 2023 | First public roadmap published — completed ahead of schedule |
| Now | **DBO Unbound** — new codebase, full control, next chapter begins |

---

## 🤝 Contributing

This is a private project. Contributions are by invitation only.

If you're interested in being part of the team, reach out through our **[Discord](https://discord.gg/fnRr3pBWg6)**.

---

<div align="center">

*DBO Unbound — Built with passion, for the players.*

</div>
